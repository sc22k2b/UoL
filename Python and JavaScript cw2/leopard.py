...
class Leopard:
    def __init__(self, filepath: str) -> None:
        self.data = []  # Sets up an instance variable to h
        # old the data (corresponding to a header)
        ...
        try:  # Sets up a try to try and read in the data
            with open(filepath, "r") as data:
                # Opens the file in read mode
                    ...
                    i = 0  # Sets up a count which will identify 
                    # if the current row consists of headers or data
                    for row in reader:
                        # Loops for the rows in the read in data
                        if(i == 0):  # Checks to see if i == 0 and
                            # therefore it is a header
                            self.header = row  # Allocates the row
                            # to the header variable
                            i = i + 1  # Increments the count
                        else:
                            self.data.append(row)  # Allocates the row
                            # to the data variable
                            i = i + 1  # Increments the count
            if not self.header:  # Checks if the header variable is
                # empty and therefore possibly there was an error
                if not self.data:  # Checks if the data variable is
                    # empty and therefore possibly there was an error
                    print("empty file.")  # As both header and data
                    # were empty the user is notified the file is empty
        except FileNotFoundError:  # Catches the exception that the
            # file doesn't exist
            print("file not found.")  # Alerts the user that the file
            # doesn't exist
    ...
    def stats(self) -> dict:
        ...
        header_length = len(self.header)  # Retrives the length of the
        # header list
        for i in range(header_length):  # Loops for the length of the
            # header list
            total = 0  # Sets up total to gather the number of data used
            mean = 0  # Sets up mean to hold the mean
            count = 0
            integer_value = False  # Holds whether the column is an integer
            # column (or float)
            ...
            for k in range(len(self.data)):  # Loops for the length of the
                ...
                if(self.data[k][i].isnumeric()):  # Checks to see if data is
                    # a number
                    integer_value = True  # Sets integer value to true as it
                    # is a number
                    total = total + 1  # Increases the total
                    data = int(self.data[k][i])  # Allocates the data
                    mean = mean + data  # Adds the data to the sum of numbers
                    # to be involved in the mean
                    if(minimum == 0):  # Checks to see if minimum hasn't
                        # changed as this would mean this is the first loop
                        # and therefore the first data is automatically the
                        # minimum
                        minimum = data  # Allocates the starting minimum
                    else:
                        if(int(minimum) > data):  # Checks to see if there is
                            # a new minimum
                            minimum = data  # Allocates the new minimum
                    if(maximum == 0):  # Checks to see if maximum hasn't
                        # changed as this would mean this is the first loop
                        # and therefore the first data is automatically the
                        # maximum
                        maximum = data  # Allocates the starting maximum
                    else:
                        if(maximum < data):  # Checks to see if there is a
                            # new maximum
